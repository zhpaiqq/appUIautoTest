...
import com.dahua.tc.gaea.report.ExtentManager;
import com.dahua.tc.gaea.report.Report;
import org.testng.*;
...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    private void init() {
        ...
        Report.setReportName(ExtentManager.projectName);
        htmlReporter.config().setDocumentTitle(Report.getReportName());
        htmlReporter.config().setReportName(ExtentManager.projectVersion);
        ...
        extent.attachReporter(htmlReporter, ExtentManager.createKlovReporter());
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                while (logIterator.hasNext()) {
                    ...
                    if (details.contains(Report.getSpiltTimeAndMsg())) {
                        String time = details.split(Report.getSpiltTimeAndMsg())[0];
                        log.setTimestamp(getTime(Long.valueOf(time)));
                        log.setDetails(details.substring(time.length() + Report.getSpiltTimeAndMsg().length()));
                    } else {
